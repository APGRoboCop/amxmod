...
static int scan_word_(const unsigned char **strp, int max, const char *const *n_full
#if USE_ABBR_NAMES
...
/*Scan1:*/
  for(ix = 0; ix < max; word_list++, ix++) {
    ...
    if(l_max < (len = strlen(word))) {
      len = l_max;
    }
    if(found_len < len && (!len || !strnicmp((const char*)*strp, word, len))) {
...
static int time_int(struct tm_int *ti, const unsigned char **buf, const char *fmt, short addthem) {
  int ii;
...
Other:
      if(isspace(ii)) {
/*SkipSp:*/
        fmt++;  skip_sp((const unsigned char **)&fmt);
        fmt--;  skip_sp(buf);
      ...
      const char *fs;
/*Fmt:*/
      switch(ii = (unsigned char)*++fmt) {
        case 0: goto FmtEnd;
        case 'a':
        case 'A':
          ti->qw = ii = scan_word(buf, 7, _lc_Wday_[_lc_Txt_], _lc_Txt_? _lc_WdayS: NULL);
          break;
        case 'h': /* STRFTIME_EXT */
        case 'b':
        case 'B':
          ti->qm = ii = scan_word(buf, 12, _lc_Month_[_lc_Txt_], _lc_Txt_? _lc_MonthS: NULL);
          break;
        case 'c':
          fs = _lc_fmt_c_[_lc_Fmt_];
strpt:    ii = time_int(ti, buf, fs, addthem);
          break;
#if STRFTIME_EXT
        case 'C': /* STRFTIME_EXT */
...
#endif
        case 'd':
          ti->qd = ii = (addthem ? scan_int2(buf) : scan_int(buf, 31)) - 1;
          break;
#if STRFTIME_EXT
        case 'D': /* STRFTIME_EXT */
          fs = _lc_fmt_xD[0];
...
CleanIp:  ti->qI = ti->qp = -1;
        ...
        case 'j':
          ...
          break;
        case 'm':
          ti->qm = ii = (addthem ? scan_int2(buf) : scan_int(buf, 12)) - 1;
          break;
        case 'M':
          ti->qM = ii = addthem ? scan_int2(buf) : scan_int(buf, 59);
          break;
#if STRFTIME_EXT
        case 'n': /* STRFTIME_EXT */
        case 't': /* STRFTIME_EXT */
...
#endif
        case 'S':
          ti->qS = ii = addthem ? scan_int2(buf) : scan_int(buf, 61);
          break;
#if STRFTIME_EXT
        case 'T': /* STRFTIME_EXT */
          fs = _lc_fmt_XT[0];
        ...
        case 'W':
          ti->qV = ii;
          ti->qU = ii = addthem ? scan_int2(buf) : scan_int(buf, 53);
          break;
        case 'w':
          ti->qw = ii = addthem ? scan_int2(buf) : scan_int(buf, 6);
          break;
        case 'x':
          fs = _lc_fmt_xD[_lc_Fmt_];
          goto strpt;
        case 'X':
          fs = _lc_fmt_XT[_lc_Fmt_];
          goto strpt;
        case 'y':
...
char *strptime(const char *buf, const char *fmt, struct tm *tm, short addthem) {
  ...
  if(0 <= ti.qM) {
    (*defoper)(&tm->tm_min, ti.qM); //tm->tm_min  = ti.qM;
  }
  if(0 <= ti.qI) {
    if(0 <= ti.qp) {
      ti.qH = ti.qI % 12 + ti.qp * 12;
    }
    else {
      (*defoper)(&tm->tm_hour, ti.qI); //tm->tm_hour = ti.qI;
    }
  }
  if(0 <= ti.qH) {
    (*defoper)(&tm->tm_hour, ti.qH); //tm->tm_hour = ti.qH;
  }
  if(0 <= ti.qZ) {
    (*defoper)(&tm->tm_isdst, ti.qZ - 1); //tm->tm_isdst = ti.qZ - 1;
  }
  if(0 <= ti.qy) {
    ti.qY = ti.qy;
  }
  if(0 <= ti.qY) {
    (*defoper)(&tm->tm_year, ti.qY += (ti.qY > 99 ? -1900 : (ti.qY < 70? 100: 0)));
  }
 					/*tm->tm_year = ti.qY +=
                              (ti.qY > 99? -1900:
                              (ti.qY < 70? 100: 0));*/
/* ti.qC = %C = Century without an Year - ignored */
  ...
  if(70 <= ti.qY && ti.qY < 200) { /* a representable year */
    ...
    if(0 <= ti.qU && 0 <= ti.qw && 0 > ti.qj) {
      ...
      switch(ti.qV) {
        case 'U': /* %U Sun first */
          ...
          break;
        case 'W': /* %W Mon first */
          ti.qj += (ti.qw + 6) % 7 - (day == 1 ? 7: (day + 6) % 7);
          break;
#if STRFTIME_EXT && 0
        case 'V': /* %V >= 4 day */
          if(ti.qU == 53) {
    ...
    if(0 <= ti.qj) {
      static int m_days[12] = { 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334 };
      int mon = (ti.qj + day) % 7;
      ...
      for(mon = 11; 0 > (day = ti.qj - (m_days[mon] + (mon > 1 && !(ti.qY & 3)))); mon--) {
        ;
      }
      if(0 > ti.qd) {
        ti.qd = day;
  ...
  if(0 <= ti.qd) {
    (*defoper)(&tm->tm_mday, ti.qd + 1);  //tm->tm_mday = ti.qd + 1;
  }
  if(0 <= ti.qm) {
    (*defoper)(&tm->tm_mon, ti.qm);  //tm->tm_mon  = ti.qm;
  }
  if(0 <= ti.qw) {
    (*defoper)(&tm->tm_wday, ti.qw);  //tm->tm_wday = ti.qw;
  }
  if(0 <= ti.qj) {
    (*defoper)(&tm->tm_yday, ti.qj);  //tm->tm_yday = ti.qj;
  }
