...
void Client_VGUIMenu(void* mValue) {
  ...
  switch(mState++) {
    case 0:
      ...
      if(mPlayer->menu != mPlayer->clearmenu) {
        mPlayer->clearmenu = 0;
        mPlayer->clearmenutime = 0.0f;
      }
      break;
    case 1:
...
void Client_CurWeapon(void* mValue) {
  ...
  switch(mState++) {
    ...
    case 2:
      ...
      if(!iState || (iId < 1 || iId >= MAX_WEAPONS)) {
        break;
      }
...
void Client_DamageEnd(void* mValue) {
	...
	if(dead && dead->death_killer) {
		g_events.parserInit(CS_DEATHMSG, &gpGlobals->time, mPlayer = nullptr, mPlayerIndex = 0, 0, nullptr);
		g_events.parseValue(dead->death_killer, MSG_ARG_SHORT);
		g_events.parseValue(dead->index, MSG_ARG_SHORT);
		g_events.parseValue(dead->death_headshot, MSG_ARG_BYTE);
		g_events.parseValue(dead->death_weapon.str());
		g_events.parseValue(dead->death_tk ? 1 : 0, MSG_ARG_BYTE);
