...
public:
	...
	public:
		...
		inline bool matchCommandLine(const char* cmd, const char* arg) {
			return (!stricmp(command.str()+prefix, cmd+prefix) && (argument.empty() || !stricmp(argument.str(), arg)));
		}
		inline bool matchCommand(const char* cmd) { return (!stricmp(command.str(), cmd)); }
    ...
    inline int getCmdTypeInt() const { return cmdtype; }
		const char* getCmdType() const;
		void setCmdType(int a);
		void setLocInfo(const char *info);
		const char* getCmdInfo();
		inline void setCmdAccess(int a, bool b) { flags = a; listable = b;}
	};
