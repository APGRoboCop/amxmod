...
public:
	...
	class CTask {
		...
		CTask(CPluginMngr::CPlugin* p, int f, int flags, int i, float base, float exec, int parlen, const cell* par, int r);
		~CTask() {
			if(param_len) delete[] param;
		}
...
public:
	...
	int changeTasks(int id, AMX* amx, float new_base);
	void clear();
