...
CTaskMngr::CTask* CTaskMngr::getFirstValidTask(CTask* h) {
	...
	while(a) {
		if(a->isRemoved()) {
			...
			if(*m_timer - *m_timeleft + 1.0f < a->base_time) {
				a = a->next;
				continue;
			}
		}
			...
			if((*m_timeleft + *m_timelimit * 60.0f) - *m_timer - 1.0f >	a->base_time) {
				a = a->next;
				continue;
			}
		}
...
CTaskMngr::CTask* CTaskMngr::findTask(int id, AMX* amx) {
	for(CTask* a = head; a; a = a->next) {
		if(!a->isRemoved() && (a->getTaskId() == id) && (!amx || (a->getPlugin()->getAMX() == amx))) {
			return a;
		}
	}
	return nullptr;
...
int CTaskMngr::changeTasks(int id, AMX* amx, float new_base) {
	int i = 0;

	for(CTask* a = head; a; a = a->next) {
		if(!a->isRemoved() && (a->getTaskId() == id) && (!amx || (a->getPlugin()->getAMX() == amx))) {
			a->base_time = new_base;
			a->exec_time = *m_timer + a->base_time;
			++i;
		}
	}
