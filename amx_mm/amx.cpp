...
#include "osdefs.h"
  ...
  #endif
#endif
#if defined __LCC__ || __GNUC__
  #include <wchar.h>    /* for wcslen() */
#endif
#include "amx.h"
#if !defined LINUX && ((defined _Windows && !defined AMX_NODYNALOAD) || defined JIT || defined JITORNOT)
  #include <windows.h>
#endif
...
static int amx_BrowseRelocate(AMX *amx)
{
  ...
  #if (defined __GNUC__ || defined ASM32 || defined JIT || defined JITORNOT) && !defined __64BIT__
    #if !defined JITORNOT
...
int AMXAPI amx_Init(AMX *amx,void *program)
{
  ...
  if (hdr->stp<=0)
    ...
    if ((hdr->flags & AMX_FLAG_COMPACT)==0) {
      ...
      while (code<(ucell *)(amx->base+(int)hdr->dat))
	    swapcell(code++);
    } /* if */
  #endif
  ...
  * (cell *)(amx->base+(int)hdr->stp-sizeof(cell)) = 0;
  ...
  #define CODESIZE_JIT    65536 //v2006.3
...
int AMXAPI amx_InitJIT(AMX *amx, void *reloc_table, void *native_code)
{
  ...
  assert(amx->sysreq_d==0); //v2006.3

  if (mprotect(ALIGN((void *)asm_runJIT), CODESIZE_JIT, PROT_READ | PROT_WRITE | PROT_EXEC) != 0)
    return AMX_ERR_INIT_JIT;

  /* copy the prefix */
  memcpy(native_code, amx->base, ((AMX_HEADER *)(amx->base))->cod);
  hdr =  static_cast<AMX_HEADER *>(native_code);
...
int AMXAPI amx_Register(AMX *amx, AMX_NATIVE_INFO *list, int number)
{
  ...
  for (i=0; i<numnatives; i++) {
    if (func->address==0) {
      ...
      if (funcptr!= nullptr)
        ...
        if(!(amx->flags & AMX_FLAG_NONATCHK)) {
  ...
  if (err==AMX_ERR_NONE)
    amx->flags|=AMX_FLAG_NTVREG; // By AMXMODX Dev Team
  return err;
}
...
// OHMYGOD
#if defined __GNUC_NEVER__ && ((!defined ASM32 && !defined JIT) || defined JITORNOT)
...
static void *amx_opcodelist_nodebug[] = {
        ...
        &&op_jump_pri,  &&op_switch,    &&op_casetbl,   &&op_swap_pri,
        &&op_swap_alt,  &&op_pushaddr,  &&op_nop,       &&op_sysreq_d,
        &&op_symtag_nodebug, &&op_break,&&op_float_mul, &&op_float_div,
        &&op_float_add, &&op_float_sub, &&op_float_to,  &&op_float_round,
        &&op_float_cmp};
  AMX_HEADER *hdr;
  AMX_FUNCSTUB *func;
  ...
  if(!(amx->flags & AMX_FLAG_NONATCHK))
	  if((amx->flags & AMX_FLAG_NTVREG)==0) // By AMXMODX Dev Team
		  return AMX_ERR_NOTFOUND;
  //v2006.3
  /*i=amx_Register(amx,NULL,0);
  if (i!=AMX_ERR_NONE)
    return i;*/
  if ((amx->flags & AMX_FLAG_RELOC)==0)
    return AMX_ERR_INIT;
...
#if defined ASM32 || defined JIT || defined JITORNOT
   ...
   */
      ...
      extern "C" cell amx_exec_asm(cell *regs,cell *retval,cell stp,cell hea);
...
int AMXAPI amx_Exec(AMX *amx, cell *retval, int index, int numparams, ...)
{
  ...
  int debug;
    ...
    #else
    #define amx_opcodelist_nodebug amx_opcodelist
    #endif
      #ifdef __WATCOMC__
  ...
  assert(amx!=NULL);
    ...
    if ((amx->flags & AMX_FLAG_BROWSE)==AMX_FLAG_BROWSE) {
      assert(sizeof(cell)==sizeof(void *));
      assert(retval!=NULL);
      //#if defined JIT || defined JITORNOT // v2006.3
...
// #else // needs another check because of JITORNOT
#endif
...
static long utf8_lowmark[5] = { 0x80, 0x800, 0x10000L, 0x200000L, 0x4000000L };
  ...
  assert(string!=NULL);
  if (value!= nullptr)      /* preset, in case of an error */
    *value=0;
  if (endptr!= nullptr)
    *endptr=string;
...
{
  assert(string!=NULL);
  if (endptr!= nullptr)     /* preset, in case of an error */
    *endptr=string;
...
int AMXAPI amx_UTF8Check(const char *string)
{
  ...
  while (err==AMX_ERR_NONE && *string!='\0')
    err=amx_UTF8Get(string,&string, nullptr);
  return err;
}
#endif /* AMX_UTF8XXX */
