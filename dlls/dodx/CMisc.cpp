...
void CPlayer::Disconnect(){
	...
	if ( ignoreBots(index/*pEdict*/) || !isModuleActive() ) // ignore if he is bot and bots rank is disabled or module is paused
		return;
...
void CPlayer::setTeamName( char *szName ){
	for (int i = 0; i < 16; ++i)
		*( (char*)pEdict->pvPrivateData + PDOFFSET_TEAMNAME + i ) = szName[i];
}
// *****************************************************
// class CMapInfo
// *****************************************************
...
void Forward::exec(int p1,int p2,int p3,int p4,int p5,int p6){
	AmxCall* a = head;
	while ( a ){
		AMX_EXEC(a->amx, NULL, a->iFunctionIdx, 6,p1, p2, p3, p4, p5, p6);
		a = a->next;
	}
}
...
void Forward::exec(int p1,int p2,int p3,int p4,int p5){
	AmxCall* a = head;
	while ( a ){
		AMX_EXEC(a->amx, NULL, a->iFunctionIdx, 5,p1, p2, p3, p4, p5);
		a = a->next;
	}
}
...
void Forward::exec(int p1,int p2,int p3){
	AmxCall* a = head;
	while ( a ){
		AMX_EXEC(a->amx, NULL, a->iFunctionIdx, 3,p1, p2, p3);
		a = a->next;
	}
}
...
void Forward::exec(int p1,int p2){
	AmxCall* a = head;
	while ( a ){
		AMX_EXEC(a->amx, NULL, a->iFunctionIdx, 2,p1, p2);
		a = a->next;
	}
}
