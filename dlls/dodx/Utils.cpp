...
//	if ( (int)dodstats_rankbots->value == 0 && ( pEnt->v.flags & FL_FAKECLIENT || ( pOther && pOther->v.flags & FL_FAKECLIENT ) ) )
bool ignoreBots (int index, edict_t *pOther){
  if((int)dodstats_rankbots->value == 0 && (MF_IsPlayerBot(index) || (pOther && MF_IsPlayerBot(GET_PLAYER_POINTER(pOther)->index))))
		return true;
	return false;
}

bool isModuleActive(){
	if ( (int)dodstats_pause->value == 0 )
		return true;
	return false;
}
